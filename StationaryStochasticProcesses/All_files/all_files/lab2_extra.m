...
for ii=1:N_sec                              % For each time frame:
    ...
    [ar_temp,e_temp] = arcov(x_temp,20);   % and save in ar_param. Why do we not need to save the first one?
    ar_param(ii,:) = [ar_temp(2:length(ar_temp))'; e_temp'];
end
